...
const states = [
    ...
    },
    {
        ...
        async handler(page, _selected, noPrompt, _defaultUsername, defaultPassword) {
            ...
            debug("Submitting form");
            const element = await page.$("span[class=submit],input[type=submit]");
    ...
    },
    {
        ...
        async handler(page, selected, noPrompt, defaultUsername, defaultPassword, code) {
            const error = await page.$(".alert-error");
            if (error) {
                debug("Found error message. Displaying");
                const errorMessage = await page.evaluate(err => err.textContent, error);
            ...
            const { verificationCode } = code || await inquirer.prompt([{
            ...
            await page.keyboard.type(code || verificationCode);
...
module.exports = {
    async loginAsync(profileName, mode, disableSandbox, noPrompt, enableChromeNetworkService, awsNoVerifySsl) {
        ...
        const samlResponse = await this._performLoginAsync(loginUrl, headless, disableSandbox, cliProxy, noPrompt, enableChromeNetworkService, profile.azure_default_username, profile.azure_default_password, profile.authenticator_key);
     ...
     * @private
     */
    async _performLoginAsync(url, headless, disableSandbox, cliProxy, noPrompt, enableChromeNetworkService, defaultUsername, defaultPassword, authenticatorKey) {
                ...
                while (true) {
                    ...
                    for (let i = 0; i < states.length; i++) {
                        ...
                        if (selected) {
                            ...
                            if(authenticatorKey) {
                                code = authenticator.generateToken(authenticatorKey);
                            }

                            await Promise.race([
                                samlResponsePromise,
                                state.handler(page, selected, noPrompt, defaultUsername, defaultPassword, code)
                            ]);
